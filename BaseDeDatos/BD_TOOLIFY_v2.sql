...
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'BD_TOOLIFY_v2')
BEGIN
    ALTER DATABASE [BD_TOOLIFY_v2] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE [BD_TOOLIFY_v2];
END
GO

CREATE DATABASE [BD_TOOLIFY_v2];
GO
...
/*Seteo de las imagenes porque estaban nullas*/
UPDATE TB_PRODUCTO
SET imagen = CONCAT('/assets/productos/P', ID_PRODUCTO, '.jpg');
GO

...
begin
	if @tipo = 'registrar'
		begin
			...
			values (@ruc,@razon,@telefono,@direccion,@idDistrito,GETDATE(),1);
			select @@IDENTITY
		end
...
begin
	...
	if @tipo = 'registrar'
		begin
			...
			values(@nombre,@descripcion,@idProveedor,@idCategoria,@precio,@stock,@imagen,GETDATE(),1)
			select @@IDENTITY
		end	
		...
		begin
			if @imagen is Null
				update TB_PRODUCTO 
					...
					PRECIO = @precio, STOCK = @stock where ID_PRODUCTO = @idProducto
			else
				update TB_PRODUCTO 
					set NOMBRE = @nombre, DESCRIPCION = @descripcion,ID_PROVEEDOR=@idProveedor,ID_CATEGORIA = @idCategoria,
					PRECIO = @precio, STOCK = @stock,IMAGEN = @imagen where ID_PRODUCTO = @idProducto
		end
	if @tipo = 'desactivar'
		begin
